...
import { BookOpen, ChevronDown, ChevronLeft, ChevronRight, Bookmark, Copy, Volume2, Pause, Play, Search, Settings, X, RotateCcw, Moon, Sun, Sparkles, Loader2, BookMarked, Square } from 'lucide-react'
    ...
    return (
        <div className={`h-screen ${t.bg} flex flex-col overflow-hidden transition-colors duration-500`}>
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
                            ...
                            <button
                                ...
                                className={`p-2 ${t.headerBtnHover} rounded-lg transition-all`}
                                title={isPanelHidden ? "Show navigation & audio" : "Hide navigation & audio"}
                            >
                                {isPanelHidden ? (
                                    <ChevronLeft size={20} className={t.headerText} />
                                ) : (
                                    <ChevronRight size={20} className={t.headerText} />
                                )}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className={`${tafsirTheme.modalBg} border-2 rounded-xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden transition-colors`}>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 sm:p-5">
                            <div className={`mb-4 p-3 rounded-lg border ${tafsirTheme.body}`}>
                                ...
                                <p className={`${arabicFont} text-2xl text-right leading-loose ${darkMode ? 'text-amber-50' : 'text-stone-900'}`}>
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                {/* Settings Sidebar */}
                <div className={`transition-all duration-300 ease-in-out ${showSettings ? 'w-72 sm:w-80 opacity-100 border-r' : 'w-0 opacity-0 border-r-0'} flex-shrink-0 z-50 overflow-hidden absolute md:relative inset-y-0 left-0 ${darkMode ? 'border-gray-800 bg-gray-900/95' : 'border-amber-200 bg-white/95'} shadow-2xl md:shadow-none`}>
                    <div className="w-72 sm:w-80 h-full p-5 sm:p-6 overflow-y-auto flex flex-col gap-6">
                        <div className="flex items-center justify-between border-b pb-4 border-amber-900/10">
                            <h3 className={`font-serif font-bold text-lg ${t.settingsTitle}`}>Pengaturan</h3>
                            <button onClick={() => setShowSettings(false)} className={`p-1 rounded-lg transition-all ${darkMode ? 'text-gray-400 hover:text-white hover:bg-gray-800' : 'text-gray-500 hover:text-gray-900 hover:bg-amber-100'}`}>
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="flex flex-col gap-5">
                            <div>
                                <label className={`text-xs font-serif font-bold ${t.label} mb-2 block uppercase tracking-wider`}>Ukuran Teks Arab</label>
                                <div className="flex gap-2">
                                    {['sm', 'md', 'lg', 'xl'].map(size => (
                                        <button
                                            key={size}
                                            onClick={() => setFontSize(size)}
                                            className={`flex-1 py-2 rounded-lg text-xs font-serif font-bold transition-all shadow-sm ${
                            ...
                            <div>
                                <label className={`text-xs font-serif font-bold ${t.label} mb-2 block uppercase tracking-wider`}>Font Arab</label>
                                <select 
                                    value={arabicFont} 
                                    onChange={(e) => setArabicFont(e.target.value)}
                                    className={`w-full rounded-lg px-3 py-2.5 text-sm font-serif font-bold transition-all shadow-sm ${
                                        darkMode ? 'bg-gray-800 text-amber-200 border border-gray-700 focus:border-amber-500' : 'bg-amber-50 text-amber-900 border border-amber-300 focus:border-amber-600'
                                    } outline-none cursor-pointer`}
                                >
                                    <option value="font-amiri">Amiri Quran</option>
                                    <option value="font-lateef">Lateef</option>
                                    <option value="font-scheherazade">Scheherazade New</option>
                                    <option value="font-noto">Noto Naskh Arabic</option>
                                    <option value="font-harmattan">Harmattan</option>
                                </select>
                            </div>
                            <div>
                                <label className={`text-xs font-serif font-bold ${t.label} mb-3 block uppercase tracking-wider`}>Opsi Tampilan</label>
                                <div className="flex flex-col gap-3">
                                    <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${darkMode ? 'border-gray-800 hover:bg-gray-800/50' : 'border-amber-200 hover:bg-amber-50'}`}>
                                        <input type="checkbox" checked={showTransliteration} onChange={(e) => setShowTransliteration(e.target.checked)} className="accent-amber-600 w-4 h-4 cursor-pointer" />
                                        <span className={`text-sm font-serif font-medium ${t.label}`}>Tampilkan Latin</span>
                                    </label>
                                    <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${darkMode ? 'border-gray-800 hover:bg-gray-800/50' : 'border-amber-200 hover:bg-amber-50'}`}>
                                        <input type="checkbox" checked={showTranslation} onChange={(e) => setShowTranslation(e.target.checked)} className="accent-amber-600 w-4 h-4 cursor-pointer" />
                                        <span className={`text-sm font-serif font-medium ${t.label}`}>Tampilkan Terjemahan</span>
                                    </label>
                                </div>
                            </div>
                            <div className="border-t border-amber-900/10 pt-5 mt-1">
                                <label className={`text-xs font-serif font-bold ${t.label} mb-2 block uppercase tracking-wider`}>Tema</label>
                                <button
                                    onClick={() => setDarkMode(!darkMode)}
                                    className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg text-sm font-serif font-bold transition-all shadow-sm ${
                                        darkMode ? 'bg-amber-700 text-white hover:bg-amber-600' : 'bg-gray-800 text-white hover:bg-gray-700'
                                    }`}
                                >
                                    {darkMode ? <><Sun size={16} /> Mode Terang</> : <><Moon size={16} /> Mode Gelap</>}
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <div className="flex-1 overflow-y-auto">
                <div className="max-w-4xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
                    {selectedSurat && (
                        <div className={`rounded-lg p-4 sm:p-6 shadow-sm mb-6 ${t.infoCard}`}>
                    ...
                    {selectedSurat?.deskripsi && (
                        <div className={`border-l-4 rounded-lg p-4 sm:p-5 mb-6 ${t.descCard}`}>
                            <span className={`text-xs uppercase font-serif font-bold ${t.descLabel} tracking-widest block mb-2`}>About This Surah</span>
                            <div className={`font-serif text-xs sm:text-sm ${t.descText} leading-relaxed text-justify`} dangerouslySetInnerHTML={{ __html: selectedSurat.deskripsi }}></div>
                        </div>
                    )}
                    {loadingAyat ? (
                        <div className="text-center py-12">
                                ...
                                return (
                                    <div key={`${selectedSurat?.nomor}-${ayat.nomorAyat}`} className={`rounded-lg transition-all p-5 sm:p-6 border-l-4 overflow-hidden relative group ${t.verseCard}`}>
                                        ...
                                        <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
                                            ...
                                            <div className="flex items-center gap-1 flex-wrap justify-end">
                                                {ayat.audio && ayat.audio[selectedReader] && (
                                                    <button onClick={() => playAyahAudio(ayat)} className={`p-1.5 sm:p-2 rounded-lg transition-all hover:scale-105 ${isPlaying ? `${t.verseAudioActive} shadow-md scale-105` : t.verseAudioInactive}`} title={isPlaying ? "Stop verse" : "Play verse"}>
                                        ...
                                        <div className={`mb-5 p-4 sm:p-6 rounded-lg border ${t.arabicBg}`}>
                                            <p className={`${arabicFont} ${fontSizeClasses[fontSize]} leading-loose text-right ${t.arabicText} tracking-wider`}>{ayat.teksArab}</p>
                                        </div>
                                        {showTransliteration && (
            ...
            {/* Controls Sidebar */}
            <div className={`transition-all duration-300 ease-in-out ${!isPanelHidden ? 'w-72 sm:w-80 opacity-100 border-l' : 'w-0 opacity-0 border-l-0'} flex-shrink-0 z-50 overflow-hidden absolute md:relative inset-y-0 right-0 ${darkMode ? 'border-gray-800 bg-gray-900/95' : 'border-amber-200 bg-white/95'} shadow-2xl md:shadow-none`}>
                <div className="w-72 sm:w-80 h-full p-5 sm:p-6 overflow-y-auto flex flex-col gap-6">
                    <div className="flex items-center justify-between border-b pb-4 border-amber-900/10">
                        <h3 className={`font-serif font-bold text-lg ${darkMode ? 'text-amber-200' : 'text-amber-900'}`}>Navigasi & Audio</h3>
                        <button onClick={() => setIsPanelHidden(true)} className={`p-1 rounded-lg transition-all ${darkMode ? 'text-gray-400 hover:text-white hover:bg-gray-800' : 'text-gray-500 hover:text-gray-900 hover:bg-amber-100'}`}>
                            <X size={20} />
                        </button>
                    </div>
                    ...
                    <div className="flex flex-col gap-5">
                        <div>
                            <label className={`block text-xs font-serif font-bold ${t.label} mb-2 uppercase tracking-widest`}>Search Surah</label>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className={`w-full rounded-lg pl-9 pr-4 py-2.5 focus:outline-none text-sm transition-colors shadow-sm ${t.searchInput}`}
                                />
                            </div>
                            {searchQuery && (
                                <div className={`mt-2 rounded-lg border max-h-40 overflow-y-auto shadow-sm ${t.searchDropdown}`}>
                        ...
                        <div>
                            <label className={`block text-xs font-serif font-bold ${t.label} mb-2 uppercase tracking-widest`}>Reciter</label>
                            <select
                                value={selectedReader}
                                onChange={(e) => setSelectedReader(e.target.value)}
                                className={`w-full rounded-lg px-3 py-2.5 focus:outline-none text-sm font-serif transition-colors shadow-sm cursor-pointer ${t.select}`}
                            >
                        ...
                        <div className={`pt-4 border-t ${darkMode ? 'border-amber-900/20' : 'border-amber-100'}`}>
                            <label className={`block text-xs font-serif font-bold ${t.label} mb-3 uppercase tracking-widest`}>Surah Control</label>
                            <div className="grid grid-cols-2 gap-2 mb-3">
                                <button onClick={handlePrevSurat} disabled={selectedSurat?.nomor === 1} className={`flex items-center justify-center gap-1 px-3 py-2 rounded-lg font-medium transition-all text-xs font-serif uppercase tracking-wide disabled:cursor-not-allowed shadow-sm ${t.navBtn}`}>
                                    <ChevronLeft size={14} /> Prev
                                </button>
                                <button onClick={handleNextSurat} disabled={selectedSurat?.nomor === 114} className={`flex items-center justify-center gap-1 px-3 py-2 rounded-lg font-medium transition-all text-xs font-serif uppercase tracking-wide disabled:cursor-not-allowed shadow-sm ${t.navBtn}`}>
                                    Next <ChevronRight size={14} />
                                </button>
                            </div>
                            {selectedSurat?.audioFull && selectedSurat.audioFull[selectedReader] && (
                                <button onClick={playFullSurah} className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-medium transition-all text-sm font-serif uppercase tracking-wide shadow-sm ${isPlayingFullSurah ? t.stopBtn : t.playBtn}`}>
                                    {isPlayingFullSurah ? <><Pause size={16} /> Stop Full Surah</> : <><Play size={16} /> Play Full Surah</>}
                                </button>
                            )}
                        </div>
            ...
            {(showSettings || !isPanelHidden) && (
                <div 
                    className="absolute inset-0 z-40 bg-black/40 md:hidden backdrop-blur-sm transition-opacity" 
                    onClick={() => {
